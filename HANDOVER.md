...
**FINISHED. 621 tests pass under `colcon test` with 5 documented skips, ruff is clean,
every world parses and every model it includes is installed, and the container
builds from a clean base and runs the same suite to the same result: 485 passed and 5 skipped, on the host and in the container, verified after
...
which for most of this project's life was untrue. See V-50. 66 numbered findings in
`docs/validation.md`.
