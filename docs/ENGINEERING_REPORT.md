...
The evidence behind [the README](../README.md): every measurement, how each subsystem is built,
the instruments that produced the numbers, and the decisions that were expensive to make. The
README is the four minute version of this file.

Each figure here cites a numbered entry in [validation.md](validation.md), which is the laboratory
notebook and stays the primary record. The nine faults worth reading on their own are in
...
[ADR 0003](adr/0003-three-tier-simulation.md): a fleet tier of 3 to 5 robots with scanners
...
any future throughput number optimistic. Recorded in [docs/validation.md](validation.md)
...
and the same arc now returns 0.06 m. See V-05 in [docs/validation.md](validation.md).
...
claim and V-06 in [docs/validation.md](validation.md) says so, with the reason and the
...
[docs/validation.md](validation.md) has the analysis.
...
[docs/validation.md](validation.md) has the numbers and the measurement bug that nearly
...
[docs/safety_concept.md](safety_concept.md).
...
[docs/validation.md](validation.md) has them.
...
validation in [docs/validation.md](validation.md): three published figures and three model
...
| `measure_braking.py` | how far the vehicle travels after it is told to stop | the field sizing rests on it, V-60 |
| `measure_load.py` | whether an unsecured load stays where it was put | a welded link cannot answer it, V-61 |
| `measure_docking.py` | how accurately the vehicle parks, and what limits it | separates the tolerance from localisation, V-62 |
...
| # | Decision |
|---|---|
| [0001](adr/0001-ros2-jazzy-and-gazebo-harmonic.md) | ROS 2 Jazzy and Gazebo Harmonic as the target |
| [0002](adr/0002-mir250-class-reference-platform.md) | A MiR250-class reference platform, chosen on provenance |
| [0003](adr/0003-three-tier-simulation.md) | Three simulation tiers, so the CPU budget is explicit |
| [0004](adr/0004-cpp-and-python-split.md) | C++ for hot paths and plugins, Python for tooling |
| [0005](adr/0005-scanner-for-safety-cameras-for-navigation.md) | The scanner is the safety sensor, the cameras are navigation sensors |
| [0006](adr/0006-ground-truth-is-an-oracle-not-an-input.md) | Ground truth is an evaluation oracle and never an input |
...
Results are measured, not asserted. Any performance number in this repository names the machine it was
...
model and the reference disagree, [docs/validation.md](validation.md) says so and says which
...
**A 52 second demo is at [docs/media/demo.mp4](media/demo.mp4)**, showing the survey building
