...
def build_truth_map(derived, iy, platform):
    ...
    pgm = b'P5\n' + f'{w} {h}\n255\n'.encode() + bytes(grid)
    meta = {
        'image': f'test_track_truth.{platform}.pgm',
        'resolution': res,
        'origin': [0.0, 0.0, 0.0],
        'negate': 0,
...
def build_keepout_mask(iy, platform):
    ...
    meta = {
        'image': f'keepout_mask_test_track.{platform}.pgm',
        'resolution': MASK_RES,
...
def main():
    ...
    iy = interior_y(spec)
    pgm, meta, (mw, mh) = build_keepout_mask(iy, args.platform)
    (maps / f'keepout_mask_test_track.{args.platform}.pgm').write_bytes(pgm)
    (maps / f'keepout_mask_test_track.{args.platform}.yaml').write_text(
    ...
    print(f'wrote {maps / f"keepout_mask_test_track.{args.platform}.yaml"}  ({mw} x {mh} cells, all free)')

    tpgm, tmeta, (tw, th) = build_truth_map(derived, iy, args.platform)
    (PKG / 'maps' / f'test_track_truth.{args.platform}.pgm').write_bytes(tpgm)
    (PKG / 'maps' / f'test_track_truth.{args.platform}.yaml').write_text(
    ...
    print(f'wrote {PKG / "maps" / f"test_track_truth.{args.platform}.yaml"}  ({tw} x {th} cells)')
